...
export const TextFieldInput = <T extends ValidComponent = "input">(
  ...
  return (
    <TextFieldPrimitive.Input
      ...
      class={cx(
        "h-10 w-full min-w-0 rounded-md border border-border bg-card px-3 py-2 text-base transition-[border-color,box-shadow] outline-none selection:bg-primary selection:text-primary-foreground file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground hover:border-input disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm dark:bg-input/30",
...
export const Input = (props: InputProps) => {
  const [, rest] = splitProps(props as InputProps, ["class"])
