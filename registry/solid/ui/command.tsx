...
import { SearchIcon } from "@/registry/solid/lib/icons"
...
function CommandInput(props: JSX.InputHTMLAttributes<HTMLInputElement>) { const [l,r]=splitProps(props,["class"]); return <div data-slot="command-input-wrapper" class="flex h-9 items-center gap-2 border-b px-3"><SearchIcon class="size-4 shrink-0 opacity-50"/><input data-slot="command-input" class={cn("flex h-10 w-full rounded-md bg-transparent py-3 text-sm outline-hidden placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50",l.class)} {...r}/></div> }
function CommandList(props: JSX.HTMLAttributes<HTMLDivElement>) { const [l,r]=splitProps(props,["class"]); return <div data-slot="command-list" class={cn("max-h-[300px] overflow-x-hidden overflow-y-auto",l.class)} {...r}/> }
...
function CommandItem(props: JSX.HTMLAttributes<HTMLDivElement>) { const [l,r]=splitProps(props,["class"]); return <div data-slot="command-item" class={cn("relative flex cursor-default items-center gap-2 rounded-md px-2 py-1.5 text-sm outline-hidden select-none hover:bg-primary/10",l.class)} {...r}/> }
